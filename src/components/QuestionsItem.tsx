import QuestionsDifficulty from "./QuestionsDifficulty"
import Badge from "./UI/Badge"

const QuestionsItem = () => {
   return (
      <div className="rounded-3xl bg-indigo-400 p-4 text-sm text-white w-full mt-2.5">
         <p className="text-white text-sm pb-4">Что такое `null` и `undefined` в JavaScript?</p>
         <div className="flex justify-between">
            <QuestionsDifficulty difficulty={3} />
            <Badge pointer={true}>
               <svg width="12" height="24" viewBox="0 0 12 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M10.157 12.711L4.5 18.368L3.086 16.954L8.036 12.004L3.086 7.054L4.5 5.64L10.157 11.297C10.3445 11.4845 10.4498 11.7388 10.4498 12.004C10.4498 12.2692 10.3445 12.5235 10.157 12.711Z" fill="white" />
               </svg>
            </Badge>
         </div>
      </div>
   )
}

export default QuestionsItem